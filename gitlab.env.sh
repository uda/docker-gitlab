...
# don't lose it. **If you lose or change this secret, 2FA will stop working for all users.** You can generate one using
...
# See the list of [acceptable values](http://api.rubyonrails.org/classes/ActiveSupport/TimeZone.html).
GITLAB_TIMEZONE=
...
# The incoming email address for reply by email. Defaults to the value of `IMAP_USER`, else defaults to
# `reply@example.com`. Please read the [reply by email](http://doc.gitlab.com/ce/incoming_email/README.html)
# documentation to curretly set this parameter.
...
GITLAB_BUILDS_DIR=
# The repository downloads directory. A temporary zip is created in this directory when users click **Download Zip** on
# a project. Defaults to `/home/git/data/tmp/downloads`.
...
# [See](http://doc.gitlab.com/ce/raketasks/backup_restore.html#create-a-backup-of-the-gitlab-system)
GITLAB_BACKUP_SKIP=
...
# Defaults to `true`. See [#138](https://github.com/sameersbn/docker-gitlab/issues/138) for use case scenario.
NGINX_HSTS_ENABLED=
...
# `github`, `bitbucket`, `gitlab`, `google_oauth2`, `facebook`, `twitter`, `saml`, `crowd`, `auth0` and `azure_oauth2`.
# No default.
...
# The account is created automatically when authentication is successful. Accepted values are `cas3`, `github`,
# `bitbucket`, `gitlab`, `google_oauth2`, `facebook`, `twitter`, `saml`, `crowd`, `auth0` and `azure_oauth2`. No default.
OAUTH_ALLOW_SSO=
...
# automatically link the omniauth identity with the LDAP account. Defaults to `false`.
OAUTH_AUTO_LINK_LDAP_USER=
# Allow users with existing accounts to login and auto link their account via SAML login,
# without having to do a manual login first and manually add SAML. Defaults to `false`.
...
# providers will have access internal projects. Accepted values are `cas3`, `github`, `bitbucket`, `gitlab`,
# `google_oauth2`, `facebook`, `twitter`, `saml`, `crowd`, `auth0` and `azure_oauth2`. No default.
...
# The name of your application. When `GITLAB_HTTPS=true`,
# defaults to `https://${GITLAB_HOST}` else defaults to `http://${GITLAB_HOST}`.
OAUTH_SAML_ISSUER=
...
# See [GitLab documentation](http://doc.gitlab.com/ce/integration/saml.html#attribute_statements) for more details.
...
# This can be used for [Libravatar integration](http://doc.gitlab.com/ce/customization/libravatar.html).
...
AWS_BACKUP_BUCKET=
# Enables mulitpart uploads when file size reaches a defined size.
# See at [AWS S3 Docs](http://docs.aws.amazon.com/AmazonS3/latest/dev/uploadobjusingmpu.html)
AWS_BACKUP_MULTIPART_CHUNK_SIZE=

# Location of custom `robots.txt`. Uses GitLab's default `robots.txt` configuration by default.
# See [www.robotstxt.org](http://www.robotstxt.org) for examples.
GITLAB_ROBOTS_PATH=
